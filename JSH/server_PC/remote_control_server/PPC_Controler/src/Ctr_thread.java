...
public class Ctr_thread implements Runnable {
	...
	public Ctr_thread(Socket sock, HashMap<String, PrintWriter> hm, ArrayList<Middleserver> server_count) {
		...
		if (firstmsg[0].equals("connection_p")) {
			...
			if (hm.get(id) != null) {
				...
				System.out.println(miniserver.getIp() + "/" + miniserver.getPort());
				pw.flush();
				pw.println("connect/" + miniserver.getIp() + "/" + miniserver.getPort());
				pw.flush();
				PrintWriter pcpw = hm.get(id);
				System.out.println(pcpw.toString());
				pcpw.flush();
				pcpw.println("connect/" + miniserver.getIp() + "/" + miniserver.getPort());
				pcpw.flush();
				hm.remove(id);
			} else {
			...
			middleservers.add(new Middleserver(ip, 1525));
			// }
		}
	...
	public void run() {
		 ...
		 * msg 형식
		 * 
		 * relay서버일경우 relay server/connection termination : relay 서버와 연결종료 relay
		 * server/increase/ip : 해당 ip의 relay 서버의 인원이 증가 relay server/decrease/ip : 해당
		 * ip의 relay 서버의 인원이 감소
		 * 
		 * client일경우 client/success : 해당 클라이언트가 중계서버 연결에 성공 client/fail : 해당 클라이언트가 중계서버
		 * 연결에 실패
		 * 
		 */
		try {
