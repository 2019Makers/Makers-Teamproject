package org.jinooworld.androidtopc;
                ...
                if(isConnected && out!=null){
                    switch(event.getAction()){
                        ...
                        case MotionEvent.ACTION_MOVE:
                            ...
                            if(disX !=0|| disY !=0){
                               // out.println(disX +","+ disY); //send mouse movement to server
                                sendThread = new SendThread(socket,disX,disY,null);
                                sendThread.start();
                        ...
                        case MotionEvent.ACTION_UP:
                            ...
                            if(!mouseMoved){
                                sendThread = new SendThread(socket,disX,disY,"action_up");
                            }
                    }
                }
                return true;
...
//            connectPhoneTask.execute("8998"); //try to connect to server in another thread
//            return true;
//        }
        ...
        protected Boolean doInBackground(String... params) {
            ...
            try {
              //  InetAddress serverAddr = InetAddress.getByName(params[0]);
                socket = new Socket("117.17.145.84", 8998);//Open socket on server IP and port
            } catch (IOException e) {
    ...
    class SendThread extends Thread {
        ...
        public SendThread(Socket socket , float x, float y, String msg) {
            this.socket = socket;
            this.x = x;
            this.y = y;
            this.msg =msg;
        ...
        public void run(float x, float y) {
            try {
                if(msg ==null)
                    output.writeUTF(String.valueOf(x)+","+ String.valueOf(y));
                else if(msg == "action_up")
                    output.writeUTF("action_up");
                output.flush();
